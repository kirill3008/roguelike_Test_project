...
class MainWidget(FloatLayout):
    def __init__(self):
        ...
        self.map = GridLayout(cols=16,rows=12,pos_hint={'center_x':0.5,'center_y':0.5},size_hint = (1,1))
        ...
        self.player_y = 0.5
        self.player_x = 0.5
        self.player = Image(size_hint=(1/16,1/12),pos_hint = {'center_y':self.player_y, 'center_x':self.player_x},source='sprites/player.gif',allow_stretch=True,anim_delay=0.25)
    ...
    def _on_keyboard_down(self, keyboard, keycode, text, modifiers):
        if keycode[1] == 'w' or keycode[1] == 'up':
            ...
            self.player_x -=1/32
            self.player_x = max(3/32,self.player_x)
            self.update_pos()
        elif keycode[1] == 'd' or keycode[1] == 'right':
            self.player_x +=1/32
            self.player_x = min(29/32,self.player_x)
            self.update_pos()
        return True
